...
Проект расширяет GitFlame AI-функциональностью для работы с репозиториями. При этом команда не имеет прямого Accessа к внутреннему коду GitFlame, поэтому в Sprint 1 мы проектируем не прямое изменение GitFlame, а внешний интеграционный сервис и контракты взаимодействия.
...
Так как у команды нет Accessа к внутреннему коду GitFlame, в Sprint 1 необходимо явно зафиксировать границы системы:
...
\begin{itemize}
    ...
    \item Git branch/PR workflow в Sprint 1 реализуется как mock или contract-level service, потому что реальный GitFlame API может быть неAccessен.
\end{itemize}
...
Важно: \texttt{.yml} хранит только настройки анализа и поведения системы, Accessные пользователю. Он не содержит выбор ML-моделей и не содержит выбор формата плана. Модели выбираются нашей системой, а план всегда возвращается в формате \texttt{Markdown}.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item границы интеграции с GitFlame без Accessа к его внутреннему коду.
    \end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item сделать mock-ответы, так как реальный GitFlame API может быть неAccessен.
    \end{itemize}
